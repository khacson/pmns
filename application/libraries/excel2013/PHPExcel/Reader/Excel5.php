	...
	private function _readSummaryInformation()
	{
		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($id) {
				...
				case 0x10:	//	Number of friendlyURL
					//	Not supported by PHPExcel
					break;
...
//		$recordData = substr($this->_data, $this->_pos + 4, $length);
	...
	private function _readSst()
	{
		...
		for ($i = 0; $i < $nm; ++$i) {
			...
			// number of friendlyURL in the Unicode string
			...
			if ($pos + $len <= $limitpos) {
				...
				// remaining friendlyURL in Unicode string
				$charsLeft = $numChars - (($isCompressed) ? $bytesRead : ($bytesRead / 2));

				$pos = $limitpos;

				// keep reading the friendlyURL
				while ($charsLeft > 0) {
...
//		echo '<br />';
	...
	private function _readHyperLink()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($hasDesc) {
				...
				// offset: 36; size: var; character array of description text, no Unicode string header, always 16-bit friendlyURL, zero terminated
			...
			case 'URL':
				...
				$offset += 4;
				// offset: var; size: $us; character array of the URL, no Unicode string header, always 16-bit friendlyURL, zero-terminated
			...
			if ($hasText) {
				...
				$offset += 4;
				// offset: var; size: var; character array of the text mark without the # sign, no Unicode header, always 16-bit friendlyURL, zero-terminated
	...
	private function _readDataValidation()
	{
		...
		try {
			...
			// in list type validity, null friendlyURL are used as item separators
...
//			var_dump($bcBitCount);
		...
		if (isset($this->_ref[$index])) {
			...
			switch ($type) {
				case 'internal':
					...
					// if there are special friendlyURL, we need to enclose the range in single-quotes
					// todo: check if we have identified the whole set of special friendlyURL
					// it seems that the following friendlyURL are not accepted for sheet names
	...
	private function _readByteStringShort($subData)
	{
		...
		// offset: 1: size: var; character array (8-bit friendlyURL)
	...
	private function _readByteStringLong($subData)
	{
		...
		// offset: 2: size: var; character array (8-bit friendlyURL)
