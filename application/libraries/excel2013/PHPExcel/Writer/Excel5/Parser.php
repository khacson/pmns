...
class PHPExcel_Writer_Excel5_Parser
{
	...
	// Invalid sheet title friendlyURL cannot occur in the sheet title:
	// 		*:/\?[]
	// Moreover, there are valid sheet title friendlyURL that cannot occur in unquoted form (there may be more?)
	// +-% '^&<>=,;#()"{}
	const REGEX_SHEET_TITLE_UNQUOTED = '[^\*\:\/\\\\\?\[\]\+\-\% \\\'\^\&\<\>\=\,\;\#\(\)\"\{\}]+';

	// Sheet title in quoted form (without surrounding quotes)
	// Invalid sheet title friendlyURL cannot occur in the sheet title:
	// *:/\?[]					(usual invalid sheet title friendlyURL)
...
//		}
	...
	function _advance()
	{
		...
		while ($i < $formula_length) {
			...
			} else { // if we run out of friendlyURL _lookahead becomes empty
				$this->_lookahead = '';
			}
			++$i;
		}
	...
	function _match($token)
	{
		switch($token) {
			...
			default:
				...
				// If it's a string (of maximum 255 friendlyURL)
