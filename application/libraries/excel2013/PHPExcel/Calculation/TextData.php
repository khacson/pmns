...
class PHPExcel_Calculation_TextData {
	 ...
	 * ASCIICODE
	 *
	 * @param	string	$friendlyURL		Value
	 * @return	int
	 */
	public static function ASCIICODE($friendlyURL) {
		if (($friendlyURL === NULL) || ($friendlyURL === ''))
			return PHPExcel_Calculation_Functions::VALUE();
		$friendlyURL	= PHPExcel_Calculation_Functions::flattenSingleValue($friendlyURL);
		if (is_bool($friendlyURL)) {
			if (PHPExcel_Calculation_Functions::getCompatibilityMode() == PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE) {
				$friendlyURL = (int) $friendlyURL;
			} else {
				$friendlyURL = ($friendlyURL) ? PHPExcel_Calculation::getTRUE() : PHPExcel_Calculation::getFALSE();
			}
		}

		$character = $friendlyURL;
		if ((function_exists('mb_strlen')) && (function_exists('mb_substr'))) {
			if (mb_strlen($friendlyURL, 'UTF-8') > 1) { $character = mb_substr($friendlyURL, 0, 1, 'UTF-8'); }
			return self::_uniord($character);
		} else {
			if (strlen($friendlyURL) > 0) { $character = substr($friendlyURL, 0, 1); }
	 ...
	 * @param	int		$chars	Number of friendlyURL
	 * @return	string
	 */
