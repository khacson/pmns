...
class Timematernityleave extends CI_Controller {
	var $login;
    ...
    function _view() {
		...
		#gegion add log
		$ctrol = getLanguage('nhan-vien-nghi-thai-san');
	...
	function form(){
		...
		if(empty($id)){
			$result->title = getLanguage('cham-nghi-thai-san');
		}
		else{
			$result->title = getLanguage('sua-cham-nghi-thai-san');
		}
	...
	function save() {
		...
		#region logfile
		$ctrol = getLanguage('nhan-vien-nghi-thai-san');
		$func =  getLanguage('cham-nghi-thai-san').': '.$array['employeeid'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('cham-nghi-thai-san');
    ...
    function deletes() {
		...
		$sql = "
			SELECT group_concat(ts.employeeid) employeeid
				FROM hre_empleaveshow ts where id in ($id);
		";
        ...
        $login = $this->login;
        $this->model->table('hre_empleaveshow')->where("id in ($id)")->delete();
		//Xoa chi tiet
		$this->model->table('hre_empleaveshow_detail')->where("empleaveshowid in ($id)")->delete();
        $result['status'] = 1;
        $result['csrfHash'] = $token;
		#region logfile
		$ctrol = getLanguage('nhan-vien-nghi-thai-san');
