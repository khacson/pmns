	  ...
	  <div class="box-tools pull-right">
		   <ul class="button-group pull-right btnpermission">
				...
				<li id="save" data-toggle="modal" data-target="#myModalFrom">
					...
					<?=getLanguage('cham-nghi-thai-san');?>
					</button>
				</li>
				<?php }?>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_ts.time_start"><?=getLanguage('tu-ngay')?></th>
						<th id="ord_ts.time_end"><?=getLanguage('den-ngay')?></th>
						<th id="ord_ts.holidays_count_month"><?=getLanguage('so-thang')?></th>
						<th id="ord_d.departmanet_name"><?=getLanguage('phong-ban')?></th>
						<th id="ord_p.position_name"><?=getLanguage('chu-vu')?></th>
						<th id="ord_dg.departmentgroup_name"><?=getLanguage('to-nhom')?></th>
						<th></th>
						<th></th>
					</tr>
				</table>
					...
					<tr class="row-searach">
						...
						<td>
							<div id="click_time_start" class="input-group date" data-provide="datepicker"  data-date-format="dd/mm/yyyy">
								<input value="" id="time_start" name="time_start" type="text" class="searchs form-control tab-event">
						...
						<td>
							<input type="text" name="holidays_count_month" id="holidays_count_month" class="searchs form-control " />
						</td>
						<td>
...
<div class="loading" style="display: none;">
	...
	<div class="blockUI blockMsg blockElement" style="width: 30%;position: absolute;top: 15%;left:35%;text-align: center;">
		<img src="<?=url_tmpl()?>img/preloader.gif" style="z-index: 299999;position: absolute;"/>
	</div>
</div> 
<!-- ui-dialog -->
<!--S Modal -->
...
<script>
	...
	function searchFunction(){
		$("#code,#fullname,#holidays_count_month").keyup(function() {
			searchList();	
		});
	...
	function save(){
		...
		if(obj.holidays_count_month == ""){
			warning('<?=getLanguage('so-thang-khong-duoc-trong');?>'); 
			return false;		
		}
		$('.loading').show();
		...
		$.ajax({
			...
			success:function(datas){
				...
				if(obj.status == -2){
					...
					error('<?=getLanguage('nhan-vien-nghi-thai-san-khong-lop-le');?>'); return false;
				}
