...
class OthercollectModel extends CI_Model
{
	...
	function findID($id) {
        ...
        $query = $this->model->table($tb['hre_othercollect'])
					  ->where('isdelete',0)
					  ->where('id',$id)
					  ->find();
        return $query;
	...
	function saves($array,$id){
		...
		$login = $this->login;
		 ...
		 $result = $this->model->table($tb['hre_othercollect'])->insert($array);	
		 return $result;
	}
	function edits($array,$id){
		...
		$array['branchid'] = $login['branchid'];
		$array['othercollect_money'] =  fmNumberSave($array['othercollect_money']);
		$this->model->table($tb['hre_othercollect'])
					->where('id',$id)
					->update($array);	
		return $id;
