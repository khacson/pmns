...
class Othercollect extends CI_Controller {
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('thu-khac');
		$func =  getLanguage('them-moi').': '.$array['othercollect_content'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('thu-khac');
		$func =  getLanguage('sua').': '.$array['othercollect_content'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('thu-khac');
		$func =  getLanguage('xoa').': '.$findID->othercollect_content;
	...
	function export(){
		...
		$sheetIndex->setCellValueByColumnAndRow(5, 1, getLanguage('ngay-thu-khac'));
		...
		foreach($query as $item){
			...
			$sheetIndex->setCellValueByColumnAndRow(6, $i, $item->othercollect_content);
			$i++;
		}
