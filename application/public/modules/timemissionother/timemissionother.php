...
class Timemissionother extends CI_Controller {
	var $login;
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns('hre_timemission_other');
		}
		$employeeid = $find->employeeid;
		$items = $this->model->table('hre_employee')->select('id,code,fullname')
								 ->where('id',$employeeid)
								 ->find();
		$data = new stdClass();
        $result = new stdClass();
    ...
    function deletes() {
		...
		$sql = "
			SELECT group_concat(ts.employeeid) employeeid
				FROM hre_timemission_other ts where id in ($id);
		";
        ...
        $login = $this->login;
        $this->model->table('hre_timemission_other')->where("id in ($id)")->delete();
        $result['status'] = 1;
        $result['csrfHash'] = $token;
		#region logfile
		$ctrol = getLanguage('nhan-vien-cong-tac');
