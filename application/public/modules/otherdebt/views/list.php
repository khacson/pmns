...
foreach ($datas as $item){ 
	...
	<tr class="content edit" id="<?=$id;?>" departmentid="<?=$item->departmentid;?>" employeeid="<?=$item->employeeid;?>">
		...
		<td class="otherdebt_money text-right"><?=number_format($item->otherdebt_money);?></td>
		<td class="otherdebt_date text-center"><?=date(configs('cfdate'),strtotime($item->otherdebt_date));?></td>
		<td class="otherdebt_content"><?=$item->otherdebt_content;?></td>
		<td class="text-center">
