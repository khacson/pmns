...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc"><?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('phu-cap');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th><?=getLanguage('phu-cap');?></th>
						<!--<th><?=getLanguage('tien-phu-cap');?></th>
						<th><?=getLanguage('loai-phu-cap');?></th>-->
						<th></th>
						<th></th>
					</tr>
				</table>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td></td>
						<td>
							<input type="text" name="allowance_name" id="allowance_name" class="searchs form-control " />
						</td>
						<td></td>
						<td></td>
					</tr>
...
<script>
	...
	function searchFunction(){
		$("#allowance_name,#allowance_money").keyup(function() {
			searchList();	
		});
	}
	...
	function save(id,func){
		...
		if(obj.allowance_name == ""){
			warning('<?=getLanguage('phu-cap-khong-duoc-trong');?>'); 
			$("#allowance_name").focus();
			return false;		
		}
		$('.loading').show();
	...
	function funcList(obj){
		$(".edit").each(function(e){
			$(this).click(function(){ 
				var allowance_name = $(".allowance_name").eq(e).text().trim();
				var allowance_money = $(".allowance_money").eq(e).text().trim();
				var id = $(this).attr('id');
				var allowance_type = $(this).attr('allowance_type');
				$("#id").val(id);	
				$("#allowance_name").val(allowance_name);
				$("#allowance_money").val(allowance_money);
				$('#allowance_type').multipleSelect('setSelects', allowance_type.split(','));				
			});
		});	
	...
	function refresh(){
		$(".loading").show();
		$(".searchs").val("");
		$('#allowance_type').multipleSelect('uncheckAll');
		csrfHash = $('#token').val();
		search = getSearch();
