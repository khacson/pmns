...
class Allowances extends CI_Controller {
    ...
    function _view() {
		...
		$data->types = $this->model->getAllowance();
		#gegion add log
		$ctrol = getLanguage('phu-cap');
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hre_allowance']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('phu-cap');
		$func =  getLanguage('them-moi').': '.$array['allowance_name'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('phu-cap');
		$func =  getLanguage('sua').': '.$array['allowance_name'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('phu-cap');
		$func =  getLanguage('xoa').': '.$findID->allowance_name;
