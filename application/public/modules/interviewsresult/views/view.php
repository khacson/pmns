<style title="" type="text/css">
	...
	table col.c14 { width: 125px; }
	table col.c15 { width: 180px; }
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th ><?=getLanguage('trinh-do-chuyen-mon');?></th>
						<th id="ord_it.description" ><?=getLanguage('ghi-chu');?></th>
						<th id="ord_it.employee_interview" ><?=getLanguage('nguoi-phong-van');?></th>
						<th id="ord_it.result_interview" ><?=getLanguage('ket-qua');?></th>
						<th id="ord_it.description_interview" ><?=getLanguage('ghi-chu');?></th>
						<th></th>
						<th></th>
					</tr>
				</table>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td></td>
						<td>
							<select class="combos" id="result_interview" name="result_interview">
								<option value="-1"><?=getLanguage('chua-phong-van')?></option>
								<option value="1"><?=getLanguage('dat')?></option>
								<option value="0"><?=getLanguage('khong-dat')?></option>
							</select>
						</td>
						<td></td>
					</tr>
...
<script>
	...
	function save(id,func){
		...
		if(obj.employee_interview == ""){
			warning('<?=getLanguage('nguoi-phong-van-duoc-trong');?>'); 
			$("#input_employee_interview").focus();
			return false;		
		}
		if(obj.result_interview == ""){
			warning('<?=getLanguage('chon-ket-qua');?>'); 
			$("#input_result_interview").focus();
			return false;		
		}
		$('.loading').show();
	...
	function init(){
		$('#result_interview').multipleSelect({
			filter: true,
			placeholder:'<?=getLanguage('chon-ket-qua')?>',
			single: false,
