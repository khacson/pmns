...
class Deletedata extends CI_Controller {
	...
	function deleteData(){
		set_time_limit(-1);
		$arrTable = $this->base_model->loadTable();
		unset($arrTable['hre_job_status']);
		unset($arrTable['hre_config']);
		unset($arrTable['hre_language']);
		unset($arrTable['hre_shif']);
		unset($arrTable['hre_branch']);
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns('hre_district');
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('xoa-du-lieu');
		$func =  getLanguage('them-moi').': '.$array['distric_name'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('xoa-du-lieu');
		$func =  getLanguage('sua').': '.$array['distric_name'];
