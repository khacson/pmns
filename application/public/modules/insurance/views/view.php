...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc"><?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('bao-hiem-kpcd');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_i.insurance_name"><?=getLanguage('ten');?></th>
						<th id="ord_i.company"><?=getLanguage('doanh-nghiep-dong');?></th>
						<th id="ord_i.workers"><?=getLanguage('nguoi-lao-dong-dong');?></th>
						<th id="ord_i.insurance_type"><?=getLanguage('loai');?></th>
						<th id="ord_i.description"><?=getLanguage('ghi-chu');?></th>
						<th></th>
						<th></th>
					</tr>
				</table>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td>
							<input type="text" name="company" id="company" class="searchs form-control " />
						</td>
						<td>
							<input type="text" name="workers" id="workers" class="searchs form-control " />
						</td>
						<td>
							<select id="insurance_type" name="insurance_type" class="combos" >
...
<script>
	...
	function searchFunction(){
		$("#insurance_name,#company,#workers,#description").keyup(function() {
			searchList();	
		});
	}
	...
	function save(id,func){
		...
		if(obj.insurance_name == ""){
			...
			$("#insurance_name").focus();
			return false;		
		}
		if(obj.company == ""){
			warning('<?=getLanguage('doanh-nghiep-dong-khong-duoc-trong');?>'); 
			return false;		
		}
		if(obj.workers == ""){
