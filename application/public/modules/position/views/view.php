...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc">
		...
		<?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('chuc-vu');?>
	  </div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_p.position_name"><?=getLanguage('chuc-vu');?></th>
						<th id="ord_p.ordering"><?=getLanguage('thu-tu');?></th>
						<th></th>
						<th></th>
					</tr>
				</table>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td></td>
						<td>
							<input type="text" name="position_name" id="position_name" class="searchs form-control" />
						</td>
						<td>
...
<script>
	...
	function searchFunction(){
		$("#position_name,#ordering").keyup(function() {
			searchList();	
		});
	}
	...
	function save(id,func){
		...
		if(obj.position_name == ""){
			warning('<?=getLanguage('chuc-vu-khong-duoc-trong');?>'); 
			$("#position_name").focus();
			return false;		
		}
		$('.loading').show();
	...
	function funcList(obj){
		$(".edit").each(function(e){
			$(this).click(function(){ 
				var position_name = $(".position_name").eq(e).text().trim();
				var id = $(this).attr('id');
				$("#id").val(id);	
