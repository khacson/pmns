...
<div class="box">
	<div class="box-header with-border">
	  ...
	  <div class="box-tools pull-right">
		<button type="button" class="btn btn-box-tool" data-widget="collapse" data-toggle="tooltip" title="<?=getLanguage('dong');?>">
	    ...
	    <div class="row">
			<div class="col-md-4">
				<div class="form-group">
					...
					<div class="col-md-8">
						<input type="text" name="bank_code" placeholder="<?=getLanguage('nhap-ma-ngan-hang');?>" id="bank_code" class="searchs form-control tab-event" />
					</div>
				</div>
			</div>
			<div class="col-md-4">
				<div class="form-group">
					...
					<div class="col-md-8">
						<input type="text" name="bank_name" placeholder="<?=getLanguage('nhap-ten-ngan-hang');?>" id="bank_name" class="searchs form-control tab-event" />
					</div>
				</div>
			</div>
			<div class="col-md-4">
...
<div class="box">
	<div class="box-header with-border">
	  <div class="brc"><?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('ngan-hang');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th><?=getLanguage('ma-ngan-hang');?></th>
						<th><?=getLanguage('ten-ngan-hang');?></th>
...
<script>
	...
	function save(id,func){
		...
		var search = getFormInput();
			...
			if(obj.bank_name == ""){
				warning('<?=getLanguage('ten-ngan-hang-khong-duoc-trong');?>'); 
				$("#bank_name").focus();
				return false;		
			}
			$('.loading').show();
	...
	function funcList(obj){
		$(".edit").each(function(e){
			$(this).click(function(){ 
				var bank_code = $(".bank_code").eq(e).text().trim();
				var bank_name = $(".bank_name").eq(e).text().trim();
