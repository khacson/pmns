 ...
 * @copyright 2016
 */
class SaladvanceModel extends CI_Model
{
	...
	function getSearch($search){
		...
		if(!empty($search['salaryadvance_money'])){
			$sql.= " and e.salaryadvance_money like '%".$search['salaryadvance_money']."%' ";	
		}
		if(!empty($search['salaryadvance_content'])){
			$sql.= " and e.salaryadvance_content like '%".$search['salaryadvance_content']."%' ";	
		}
		if(!empty($search['datecreate'])){
			...
			$sql.= " and r.salaryadvance_date >= '".fmDateSave($fromdate)." 00:00:00' ";	
			$sql.= " and r.salaryadvance_date <= '".fmDateSave($todate)." 23:59:59' ";	
		}
		return $sql;
	}
	...
	function saves($array,$id){
		...
		$login = $this->login;
		 ...
		 $result = $this->model->table($tb['hre_salaryadvance'])->insert($array);	
		 return $result;
	}
	function edits($array,$id){
		...
		$array['branchid'] = $login['branchid'];
		$array['salaryadvance_money'] =  fmNumberSave($array['salaryadvance_money']);
		$this->model->table($tb['hre_salaryadvance'])
					->where('id',$id)
					->update($array);	
		return $id;
