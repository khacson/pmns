...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc"><?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('thang-nam');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th><?=getLanguage('thang-nam');?></th>
						<th><?=getLanguage('tu-ngay');?></th>
						<th><?=getLanguage('den-ngay');?></th>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td></td>
						<td>
							<input type="text" name="monthyear" id="monthyear" class="searchs form-control tab-event" />
						</td>
						<td></td>
						<td></td>
						<td></td>
...
<script>
	...
	function save(id,func){
		...
		if(obj.monthyear == ""){
			warning('<?=getLanguage('thang-nam-khong-duoc-trong');?>'); 
			$("#input_monthyear").focus();
			return false;		
		}
		if(obj.date_start == ""){
			warning('<?=getLanguage('ngay-bat-dau-khong-duoc-trong');?>'); 
			$("#input_date_start").focus();
			return false;		
		}
		if(obj.date_end == ""){
			warning('<?=getLanguage('ngay-ket-thuc-khong-duoc-trong');?>'); 
			$("#input_date_end").focus();
			return false;		
		}
		if(obj.number_days == ""){
			warning('<?=getLanguage('ngay-cong-khong-duoc-trong');?>'); 
			$("#input_number_days").focus();
			return false;		
		}
	...
	function funcList(obj){
		$(".edit").each(function(e){
			$(this).click(function(){ 
				var monthyear = $(".monthyear").eq(e).text().trim();
