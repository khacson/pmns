...
class Province extends CI_Controller {
    ...
    function _view() {
		...
		#gegion add log
		$ctrol = getLanguage('tinh-thanh-pho');
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hre_province']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('tinh-thanh-pho');
		$func =  getLanguage('them-moi').': '.$array['province_name'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('tinh-thanh-pho');
		$func =  getLanguage('sua').': '.$array['province_name'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('tinh-thanh-pho');
		$func =  getLanguage('xoa').': '.$findID->province_name;
