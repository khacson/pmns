...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc"><?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('luong-tang-ca');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_e.overtime_name" ><?=getLanguage('luong-tang-ca');?></th>
						<th id="ord_e.overtime_pay" ><?=getLanguage('so-tien');?></th>
						<th id="ord_e.overtime_pay_type" ><?=getLanguage('loai');?></th>
						<th id="ord_e.overtime_pay_by" ><?=getLanguage('tinh-theo');?></th>
						<th></th>
						<th></th>
					</tr>
				</table>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td></td>
						<td>
							<input type="text" name="overtime_name" id="overtime_name" class="searchs form-control " />
						</td>
						<td></td>
						<td></td>
						<td></td>
...
<script>
	...
	function save(id,func){
		...
		if(obj.overtime_name == ""){
			warning('<?=getLanguage('luong-tang-ca-khong-duoc-trong');?>'); 
			$("#overtime_name").focus();
			return false;		
		}
		$('.loading').show();
	...
	function funcList(obj){
		$(".edit").each(function(e){
			$(this).click(function(){ 
				var overtime_name = $(".overtime_name").eq(e).text().trim();
				var id = $(this).attr('id');
				$("#id").val(id);	
				$("#overtime_name").val(overtime_name);	
			});
		});	
