<style title="" type="text/css">
	...
	table col.c12 { width: 160px;}
	table col.c13 { width: 60px;}
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_e.date_start"><?=getLanguage('ngay-bat-dau')?></th>
						<th id="ord_e.contrac_date"><?=getLanguage('ngay-ky-hd')?></th>
						<th ><?=getLanguage('nam-lam-viec')?></th>
						<th ><?=getLanguage('tong')?></th>
						<th ><?=getLanguage('ngay-phep')?></th>
						<th ><?=getLanguage('ngay-cong-them')?></th>
...
<script>
	...
	function save(id,func){
		...
		if(obj.bonus_holiday == ""){
			warning('<?=getLanguage('nhap-ngay-phep');?>'); 
			$("#input_bonus_holiday").focus();
			return false;		
		}
		$('.loading').show();
