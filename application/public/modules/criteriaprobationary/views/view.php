...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc"><?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('tieu-chi-danh-gia');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_e.aprobationary_name" ><?=getLanguage('tieu-chi-danh-gia');?></th>
						<th></th>
						<th></th>
					</tr>
				</table>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td></td>
						<td>
							<input type="text" name="aprobationary_name" id="aprobationary_name" class="searchs form-control " />
						</td>
						<td></td>
						<td></td>
					</tr>
...
<script>
	...
	function searchFunction(){
		$("#aprobationary_name").keyup(function() {
			searchList();	
		});
	}
	...
	function save(id,func){
		...
		if(obj.aprobationary_name == ""){
			warning('<?=getLanguage('hre_ethnic-khong-duoc-trong');?>'); 
			$("#aprobationary_name").focus();
			return false;		
		}
		$('.loading').show();
	...
	function funcList(obj){
		$(".edit").each(function(e){
			$(this).click(function(){ 
				var aprobationary_name = $(".aprobationary_name").eq(e).text().trim();
				var id = $(this).attr('id');
				$("#id").val(id);	
				$("#aprobationary_name").val(aprobationary_name);	
			});
		});	
