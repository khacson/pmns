...
class Salaryadvance extends CI_Controller {
	var $login;
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns('hre_salaryadvance');
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('tam-ung');
		$func =  getLanguage('them-moi').': '.$array['salaryadvance_content'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('tam-ung');
		$func =  getLanguage('sua').': '.$array['salaryadvance_content'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('tam-ung');
		$func =  getLanguage('xoa').': '.$findID->salaryadvance_content;
	...
	function export(){
		...
		$sheetIndex->setCellValueByColumnAndRow(5, 1, getLanguage('ngay-tam-ung'));
		...
		foreach($query as $item){
			...
			$sheetIndex->setCellValueByColumnAndRow(6, $i, $item->salaryadvance_content);
			$i++;
		}
