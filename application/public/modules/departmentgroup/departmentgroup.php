...
class Departmentgroup extends CI_Controller {
    var $phonedetail;
	var $login;
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hre_departmentgroup']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('to-nhom');
		$func =  getLanguage('them-moi').': '.$array['departmentgroup_name'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('to-nhom');
		$func =  getLanguage('sua').': '.$array['departmentgroup_name'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('to-nhom');
		$func =  getLanguage('xoa').': '.$findID->departmentgroup_name;
