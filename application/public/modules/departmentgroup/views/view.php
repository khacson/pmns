...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc"><i class="fa fa-bars"></i> <?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('to-nhom');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_dp.departmentgroup_name"><?=getLanguage('to-nhom');?></th>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td></td>
						<td>
							<input type="text" name="departmentgroup_name"  id="departmentgroup_name" class="searchs form-control" />
						</td>
						<td>
...
<script>
	...
	function searchFunction(){
		$("#departmentgroup_name").keyup(function() {
			searchList();	
		});
	}
	...
	function save(){ //
		...
		if(obj.departmentgroup_name == ""){
			warning('<?=getLanguage('phong-ban-khong-duoc-trong');?>'); 
			$("#departmentgroup_name").focus();
			return false;		
		}
		if(obj.departmentid == ""){
	...
	function funcList(obj){
		$(".edit").each(function(e){
			$(this).click(function(){ 
				var departmentgroup_name = $(".departmentgroup_name").eq(e).text().trim();
				var id = $(this).attr('id');
				var departmentid = $(this).attr('departmentid');
				$("#id").val(id);	
				$("#departmentgroup_name").val(departmentgroup_name);
