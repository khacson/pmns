...
class User extends CI_Controller {
    ...
    function _view() {
		...
		$data->groups = $this->model->getallGroup($login['companyid']);
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns('hre_users');
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;
    ...
    function save() {
		...
		if(isset($_FILES['signatures']) && $_FILES['signatures']['name'] != "") {
			$imge_name = $_FILES['signatures']['name'];
			$this->upload->initialize($this->set_upload_options());
			$image_data = $this->upload->do_upload('signatures', $imge_name); //Ten hinh 
			$array['signatures']  = $image_data;
			$resize = $this->resizeImg($image_data);	
		}
		...
		#gegion add log
		$ctrol =  getLanguage('nhan-vien');
		$func =   getLanguage('them').': '.$array['username'];
		$this->base_model->addAcction($ctrol,$func,'','');
		#end
        $result['csrfHash'] = $token;
        echo json_encode($result);
	...
	function edit() {
		...
		#gegion add log
		$ctrol =  getLanguage('nhan-vien');
		$func =   getLanguage('sua').': '.$array['username'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol =  getLanguage('nhan-vien');
		$func =   getLanguage('xoa').': '.$findID->username ;
