 ...
 * @copyright 2016
 */
class CriteriakpiModel extends CI_Model
{
	...
	function saves($array,$id){
		$tb = $this->base_model->loadTable();
		$check = $this->model->table($tb['hre_criteriakpi'])
					  ->select('id')
					  ->where('isdelete',0)
		 ...
		 $result = $this->model->table($tb['hre_criteriakpi'])->insert($array);	
		 return $result;
	}
	function edits($array,$id){
		...
		$this->model->table($tb['hre_criteriakpi'])
					->where('id',$id)
					->update($array);	
		return $id;
