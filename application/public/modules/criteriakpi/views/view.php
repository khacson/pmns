...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc"><?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('tieu-chi-kpi');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_k.kpi_code" ><?=getLanguage('ma-kpi');?></th>
						<th id="ord_k.kpi_name" ><?=getLanguage('ten-kpi');?></th>
						<th id="ord_k.kpi_point_max" ><?=getLanguage('diem-toi-da');?></th>
						<th></th>
						<th></th>
					</tr>
				</table>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td></td>
						<td>
							<input type="text" name="kpi_code" id="kpi_code" class="searchs form-control " />
						</td>
						<td>
							<input type="text" name="kpi_name" id="kpi_name" class="searchs form-control " />
						</td>
						<td>
							<input type="text" name="kpi_point_max" id="kpi_point_max" class="searchs form-control " />
						</td>
						<td></td>
						<td></td>
					</tr>
...
<script>
	...
	function searchFunction(){
		$("#kpi_code,#kpi_name,#kpi_point_max").keyup(function() {
			searchList();	
		});
	}
	...
	function save(id,func){
		...
		if(obj.kpi_code == ""){
			warning('<?=getLanguage('ma-khong-duoc-trong');?>'); 
			$("#input_kpi_code").focus();
			return false;		
		}
		if(obj.kpi_name == ""){
			warning('<?=getLanguage('ten-khong-duoc-trong');?>'); 
			$("#input_kpi_name").focus();
			return false;		
		}
		$('.loading').show();
	...
	function funcList(obj){
		$(".edit").each(function(e){
			$(this).click(function(){ 
				var kpi_code = $(".kpi_code").eq(e).text().trim();
				var kpi_name = $(".kpi_name").eq(e).text().trim();
				var kpi_point_max = $(".kpi_point_max").eq(e).text().trim();
				var id = $(this).attr('id');
				$("#id").val(id);	
				$("#kpi_code").val(kpi_code);	
				$("#kpi_name").val(kpi_name);	
				$("#kpi_point_max").val(kpi_point_max);	
			});
		});	
