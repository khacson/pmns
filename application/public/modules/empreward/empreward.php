...
class Empreward extends CI_Controller {
    var $phonedetail;
	var $login;
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hre_reward']);
			$code = '';
		}
		else{
			$code = $this->model->table($tb['hre_employee'])
								->select('code',$find->employeeid)
								->find()->code;
		}
		$data = new stdClass();
        $result = new stdClass();
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('khen-thuong');
		$func =  getLanguage('them-moi').': '.$array['reward_content'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('khen-thuong');
		$func =  getLanguage('sua').': '.$array['reward_content'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('khen-thuong');
		$func =  getLanguage('xoa').': '.$findID->reward_content;
