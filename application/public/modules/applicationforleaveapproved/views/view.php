			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td></td>
						<td>
							<select class="combos" id="approved" name="approved"  >
								<option value="1"><?=getLanguage('cho-nghi-phep')?></option>
								<option value="0"><?=getLanguage('khong-cho-nghi-phep')?></option>
							</select>
						</td>
						<td></td>
						<td></td>
...
<script>
	...
	function init(){
		...
		$('#approved').multipleSelect({
			filter: true,
			placeholder:'<?=getLanguage('chon-tinh-trang')?>',
			single: false,
			onClick: function(view){
				searchList();
			}
