...
class Trainingcourse extends CI_Controller {
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hre_trainingcourse']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('khoa-hoc');
		$func =  getLanguage('them-moi').': '.$array['trainingcourse_name'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('khoa-hoc');
		$func =  getLanguage('sua').': '.$array['trainingcourse_name'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('khoa-hoc');
		$func =  getLanguage('xoa').': '.$findID->trainingcourse_name;
