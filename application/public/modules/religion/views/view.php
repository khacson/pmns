...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc"><i class="fa fa-bars"></i> <?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('dan-toc');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_r.religion_name"><?=getLanguage('ton-giao');?></th>
						<th></th>
						<th></th>
					</tr>
				</table>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td></td>
						<td>
							<input type="text" name="religion_name" id="religion_name" class="searchs form-control" />
						</td>
						<td><td>
					</tr>
...
<script>
	...
	function save(id,func){
		...
		if(obj.religion_name == ""){
			warning('<?=getLanguage('ton-giao-khong-duoc-trong');?>'); 
			$("#religion_name").focus();
			return false;		
		}
		$('.loading').show();
	...
	function funcList(obj){
		$(".edit").each(function(e){
			$(this).click(function(){ 
				var religion_name = $(".religion_name").eq(e).text().trim();
				var id = $(this).attr('id');
				$("#id").val(id);	
