...
class Timemission extends CI_Controller {
	var $login;
    ...
    function _view() {
		...
		$data->todate = '';// date(configs('cfdate'),strtotime($todate));
		
		#gegion add log
		$ctrol = getLanguage('nhan-vien-cong-tac');
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hre_empleaveshow']);
		}
		...
		if(empty($id)){
			$result->title = getLanguage('cham-cong-tac');
		}
		else{
			$result->title = getLanguage('sua-cham-cong-tac');
		}
	...
	function save() {
		...
		//$array['departmentid'] = $login['departmentid'];
        $result['status'] = $this->model->saves($array,$id);
		#region logfile
		$ctrol = getLanguage('nhan-vien-cong-tac');
		$func =  getLanguage('cham-cong-tac').': '.$array['employeeid'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('cham-cong-tac');
    ...
    function deletes() {
		...
		$sql = "
			SELECT group_concat(ts.employeeid) employeeid
				FROM `".$tb['hre_empleaveshow']."` ts where id in ($id);
		";
        ...
        $login = $this->login;
        $this->model->table($tb['hre_empleaveshow'])->where("id in ($id)")->delete();
		//Xoa bảng chi tiết
		$this->model->table($tb['hre_empleaveshow_detail'])->where("empleaveshowid in ($id)")->delete();
        $result['status'] = 1;
        $result['csrfHash'] = $token;
		#region logfile
		$ctrol = getLanguage('nhan-vien-cong-tac');
