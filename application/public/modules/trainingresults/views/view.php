...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc"><?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('ket-qua-dao-tao');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_tn.catalogid" ><?=getLanguage('khoa-hoc');?></th>
						<th id="ord_tn.result_status" ><?=getLanguage('ket-qua');?></th>
						<th id="ord_tn.date_finish" ><?=getLanguage('ngay-hoan-thanh');?></th>
			...
			<div id="gridView">
				<table id="group"  width="100%" cellspacing="0" border="1">
					...
					<tr class="row-searach">
						...
						<td>
							<select class="combos" id="catalogid" name="catalogid">
								<?php foreach($catalogs as $item){?>
								<option value="<?=$item->id;?>"><?=$item->trainingcourse_name;?></option>
								<?php }?>
							</select>
						</td>
						<td>
							<input type="text" name="result_status" id="result_status" class="searchs form-control " />
						</td>
						<td>
							<div id="click_date_finish" class="input-group date" data-provide="datepicker"  data-date-format="dd/mm/yyyy">
								<input value="" id="date_finish" name="date_finish" type="text" class="searchs form-control tab-event">
...
<script>
	...
	function searchFunction(){
		$("#code,#fullname,#result_status,#description").keyup(function() {
			searchList();	
		});
		$('#click_date_finish').datepicker().on('changeDate', function (ev) {
			searchList();
		});
	}
	...
	function save(id,func){
		...
		if(obj.catalogid == ""){
			warning('<?=getLanguage('chon-khoa-hoc');?>'); 
			return false;		
		}
		if(obj.result_status == ""){
			warning('<?=getLanguage('ket-qua-dao-tao-khong-duong-trong');?>'); 
			return false;		
		}
		if(obj.date_finish == ""){
			warning('<?=getLanguage('ngay-hoan-thanh-khong-duong-trong');?>'); 
			return false;		
		}
		$('.loading').show();
	...
	function init(){
		$('#catalogid').multipleSelect({
			filter: true,
			placeholder:'<?=getLanguage('chon-khoa-hoc')?>',
			single: false,
