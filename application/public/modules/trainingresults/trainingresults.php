...
class Trainingresults extends CI_Controller {
	var $login;
    ...
    function _view() {
		...
		#gegion add log
		$ctrol = getLanguage('ket-qua-dao-tao');
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hre_trainingresults']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('ket-qua-dao-tao');
		$func =  getLanguage('xoa').': '.$findID->id;
