...
class Regovertime extends CI_Controller {
	var $login;
    ...
    function _view() {
		...
		$data->login = $login;
		#gegion add log
		$ctrol = getLanguage('nhan-vien-tang-ca');
    ...
    function deletes() {
		...
		$sql = "
			SELECT group_concat(ts.employeeid) employeeid
				FROM hre_regovertime ts where id in ($id);
		";
        ...
        $login = $this->login;
        $this->model->table('hre_regovertime')->where("id in ($id)")->delete();
        $result['status'] = 1;
        $result['csrfHash'] = $token;
		#region logfile
		$ctrol = getLanguage('nhan-vien-tang-ca');
	...
	function saveApproved(){
		...
		$this->model->table('hre_regovertime')
							   ->where('id',$id)
							   ->update($update);
		#end
		#region logfile
