...
class Academiclevel extends CI_Controller {
    var $phonedetail;
	var $login;
    ...
    function _view() {
		...
		#gegion add log
		$ctrol = getLanguage('trinh-do-hoc-van');
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hre_academic_level']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('trinh-do-hoc-van');
		$func =  getLanguage('them-moi').': '.$array['academic_name'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('trinh-do-hoc-van');
		$func =  getLanguage('sua').': '.$array['academic_name'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('trinh-do-hoc-van');
		$func =  getLanguage('xoa').': '.$findID->academic_name;
