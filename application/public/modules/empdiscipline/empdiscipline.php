...
class Empdiscipline extends CI_Controller {
    var $phonedetail;
	var $login;
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hre_discipline']);
			$code = '';
		}
		else{
			$code = $this->model->table($tb['hre_employee'])->select('code',$find->employeeid)->find()->code;
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('ky-luat');
		$func =  getLanguage('them-moi').': '.$array['discipline_content'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('ky-luat');
		$func =  getLanguage('sua').': '.$array['discipline_content'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('ky-luat');
		$func =  getLanguage('xoa').': '.$findID->discipline_content;
