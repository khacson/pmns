...
<div class="box mtop10">
	<div class="box-header with-border">
	  <div class="brc"><i class="fa fa-bars"></i> <?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('tinh-trang-cong-viec');?></div>
			...
			<div id="tHeader">    	
				<table width="100%" cellspacing="0" border="1" class="table ">
					...
					<tr>
						...
						<th id="ord_js.status_name"><?=getLanguage('tinh-trang-cong-viec');?></th>
						<th id="ord_js.ordering"><?=getLanguage('thu-tu');?></th>
						<th></th>
						<th></th>
					</tr>
				</table>
...
<script>
	...
	function searchFunction(){
		$("#status_name,#ordering").keyup(function() {
			searchList();	
		});
	}
	...
	function save(id,func){
		...
		if(obj.status_name == ""){
			warning('<?=getLanguage('tinh-trang-cong-viec-khong-duoc-trong');?>'); 
			$("#status_name").focus();
			return false;		
		}
		$('.loading').show();
	...
	function funcList(obj){
		$(".edit").each(function(e){
			$(this).click(function(){ 
				var status_name = $(".status_name").eq(e).text().trim();
				var id = $(this).attr('id');
				$("#id").val(id);	
