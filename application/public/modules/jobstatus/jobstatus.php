...
class Jobstatus extends CI_Controller {
    var $phonedetail;
	var $login;
    ...
    function _view() {
		...
		#gegion add log
		$ctrol = getLanguage('tinh-trang-cong-viec');
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hre_job_status']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
    ...
    function save() {
		...
		#region logfile
		$ctrol = getLanguage('tinh-trang-cong-viec');
		$func =  getLanguage('them-moi').': '.$array['status_name'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
	...
	function edit() {
		...
		#region logfile
		$ctrol = getLanguage('tinh-trang-cong-viec');
		$func =  getLanguage('sua').': '.$array['status_name'];
    ...
    function deletes() {
		...
		#region logfile
		$ctrol = getLanguage('tinh-trang-cong-viec');
		$func =  getLanguage('xoa').': '.$findID->status_name;
