...
class Employeevacation extends CI_Controller {
	var $login;
	...
	function export(){
		...
		$sheetIndex->setCellValueByColumnAndRow(3, 1, getLanguage('ngay-nghi'));
		$sheetIndex->setCellValueByColumnAndRow(4, 1, getLanguage('phong-ban'));
		$sheetIndex->setCellValueByColumnAndRow(5, 1, getLanguage('chu-vu'));
		$sheetIndex->setCellValueByColumnAndRow(6, 1, getLanguage('to-nhom'));
		...
		foreach($query as $item){
			...
			$sheetIndex->setCellValueByColumnAndRow(3, $i, $datenow);
			$sheetIndex->setCellValueByColumnAndRow(4, $i, $item->departmanet_name);
			$sheetIndex->setCellValueByColumnAndRow(5, $i, $item->position_name);
			$sheetIndex->setCellValueByColumnAndRow(6, $i, $item->departmentgroup_name);
	...
	/*function save() {
		...
		#region logfile
		$ctrol = getLanguage('nhan-vien-di-lam');
		$func =  getLanguage('cham-cong').': '.$array['code'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
    ...
    function deletes() {
		...
		$sql = "
			SELECT group_concat(ts.employeeid) employeeid
				FROM hre_timesheets ts where id in ($id);
		";
        ...
        $login = $this->login;
        $this->model->table('hre_timesheets')->where("id in ($id)")->delete();
        $result['status'] = 1;
        $result['csrfHash'] = $token;
		#region logfile
		$ctrol = getLanguage('nhan-vien-di-lam');
