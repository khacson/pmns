...
class Empleaveshow extends CI_Controller {
	var $login;
	...
	function form(){
		...
		if(empty($id)){
			$result->title = getLanguage('cham-nghi-phep');
		}
		else{
			$result->title = getLanguage('sua-cham-nghi-phep');
		}
	...
	function save() {
		...
		#region logfile
		$ctrol = getLanguage('nhan-vien-nghi-phep');
		$func =  getLanguage('cham-nghi-phep').': '.$array['employeeid'];
		$this->base_model->addAcction($ctrol,$func,'','');	
		#end
    ...
    function deletes() {
		...
		$this->model->table($tb['hre_empleaveshow_detail'])
					->where("empleaveshowid in ($id)")
					->delete();
        $result['status'] = 1;
        $result['csrfHash'] = $token;
		#region logfile
